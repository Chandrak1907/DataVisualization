function multiline(data) {
    ...
    var formatDate = d3.time.format("%Y%m"),
        ...
        formatOutput0 = function(d) { return formatDate(d.Month) + " - " + "Mortgage: " + formatting(d.Mortgage); },
        formatOutput1 = function(d) { return formatDate(d.Month) + " - " + "Credit: " + d.Credit; },
        formatOutput2 = function(d) { return formatDate(d.Month) + " - " + "Debt: " + formatting(d.Debt); };
    ...
    var focus_line0 = d3.svg.line()
        ...
        .x(function(d) { 
            console.log("Data ",d," -- ",focus_x(d.Month), " == ", focus_y0(d.Mortgage));
            return focus_x(d.Month); })
        .y(function(d) { return focus_y0(d.Mortgage); });

    var focus_line1 = d3.svg.line()
        .interpolate("linear")
        .x(function(d) { return focus_x(d.Month); })
        .y(function(d) { return focus_y0(d.Credit); });

    var focus_line2 = d3.svg.line()
        .interpolate("linear")
        .x(function(d) { return focus_x(d.Month); })
        .y(function(d) { return focus_y0(d.Debt); });


    // add lines to context plot
    var context_line0 = d3.svg.line()
        .x(function(d) { return context_x(d.Month); })
        .y(function(d) { return context_y0(d.Mortgage); });

    var context_line1 = d3.svg.line()
        .x(function(d) { return context_x(d.Month); })
        .y(function(d) { return context_y0(d.Credit); });

    var context_line2 = d3.svg.line()
        .x(function(d) { return context_x(d.Month); })
        .y(function(d) { return context_y0(d.Debt); });
    ...
    focus_y0.domain([0, d3.max(data, function(d) { return d.Mortgage; })]).nice();
    ...
    function mousemove() {
        ...
        focus.select("circle.y0").attr("transform", "translate(" + focus_x(d.Month) + "," + focus_y0(d.Mortgage) + ")");
        focus.select("text.y0").attr("transform", "translate(" + focus_x(d.Month) + "," + focus_y0(d.Mortgage) + ")")
            .text(formatOutput0(d));     // text for hover tooltip
        focus.select("circle.y1").attr("transform", "translate(" + focus_x(d.Month) + "," + focus_y0(d.Credit) + ")");
        focus.select("text.y1").attr("transform", "translate(" + focus_x(d.Month) + "," + focus_y0(d.Credit) + ")")
            .text(formatOutput1(d));   // text for hover tooltip
        focus.select("circle.y2").attr("transform", "translate(" + focus_x(d.Month) + "," + focus_y0(d.Debt) + ")");
        focus.select("text.y2").attr("transform", "translate(" + focus_x(d.Month) + "," + focus_y0(d.Debt) + ")")
        ...
        focus.select(".y0").attr("transform", "translate(" + focus_width * -1 + ", " + focus_y0(d.Mortgage) + ")")
               .attr("x2", focus_width + focus_x(d.Month));
        focus.select(".y1").attr("transform", "translate(" + focus_width * -1 + ", " + focus_y0(d.Credit) + ")")
               .attr("x2", focus_width + focus_x(d.Month));
        focus.select(".y2").attr("transform", "translate(" + focus_width * -1 + ", " + focus_y0(d.Debt) + ")")
               .attr("x2", focus_width + focus_x(d.Month));
    }
